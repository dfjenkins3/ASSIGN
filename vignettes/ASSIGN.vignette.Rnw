...
\begin{enumerate}
  ...
  irrelevant signature genes toward zero. The parameter regularization constrains the
  pathway signature to a small group of genes, thus, making the results more
...
training dataset is a G (Number of genomic measures) x N (number of samples in
pathway perturbation experiments) matrix, including five oncogenic pathways:
...
purturbation experiment. For example, we specify the column indices of the 10 RAS
control samples to be 1:10, and column indices of 10 RAS activated samples to be
39:48. The test dataset is a G (Number of genomic measures) x N (number of
patient samples) matrix. The test data labels denote the classes of the N test
samples. In our example, test samples 1-53 are adenocarcinoma and samples
54-111 are squamous cell carcinoma. We specify "Adeno" and "Squamous" in the
vector of test data labels. Note that the test data labels are optional.
\Rpackage{ASSIGN} outputs additional validation plots to evaluate classification
accuracy when test data labels are provided.
...
\begin{itemize}
  ...
  \item \textbf{signature\_heatmap\_testset\_prior.pdf}: heatmaps of expression
  level of prior signature genes in training samples.
...
\begin{itemize}
  ...
  \item \textbf{signature\_heatmap\_trainingset.pdf}: heatmaps of expression
...
\begin{itemize}
  \item \textbf{signature\_heatmap\_testset\_posterior.pdf}: heatmaps of
...
a temporary dirctory "tempdir" and load training and test datasets. The
...
We first run \Rfunction{assign.preprocess} function on the input datasets. When
the genomic measures (i.g., gene expression profiles) of training samples are
provided, but predetermined pathway signature gene lists are not provided,
\Rfunction{assign.preprocess} function utilizes a Bayesian univariate regression
module to select a gene set (usually 50-200 but can be specified by the user)
based on the absolute value of the regression coefficient (fold change) and the
posterior probability of the variable to be selected (statistical significance).
Since we have no predetermined gene lists to provide, we leave the
\Rfunarg{geneList} option as default NULL. Here we specify 200 signature genes
for each of the five pathways.
...
signatures, although not significantly differently expressed, need to be
included for the prediction purpose. In this case we specify
\Rfunarg{trainingData} and \Rfunarg{geneList} when BOTH of the training dataset
and predetermined signature gene list are available.
...
(S\_matrix) and the probability signature matrix (Pi\_matrix) and differential
expressed gene lists of each pathways (diffGeneList). The details of the
assign.preprocess output are described in the Value section of the manual page
of \Rfunction{assign.preprocess} function. The output data of
...
2000 iterations for each parameters. The details of \Rfunction{assign.mcmc}
...
through \Rfunction{assign.convergence}. The \Rfunarg{burn\_in} is set
default 0, so that the trace plot starts from the first iteration. The
additional iteration can be specified if the MCMC chain is not converged
in 2000 iterations.
...
The \Rfunction{assign.convergence} function returns the a vector of the
...
parameters. The details of \Rfunction{assign.summary} output are described in
...
\begin{enumerate}
  \item \textbf{pathway\_activity\_trainingset.csv}: \Rpackage{ASSIGN} predicted
  pathway acitivity in training samples.
  \item \textbf{signature\_heatmap\_trainingset.pdf}: heatmaps of expression
  level of signature genes in training samples.
  \item \textbf{pathway\_activity\_scatterplot\_trainingset.pdf}: scatterplot of
  pathway activity in training samples.
...
\begin{enumerate}
  \item \textbf{pathway\_activity\_testset.csv}: \Rpackage{ASSIGN} predicted
  pathway acitivity in test samples.
  \item \textbf{signature\_heatmap\_testset\_prior.pdf}: heatmaps of expression
  level of prior signature genes in training samples.
  \item \textbf{signature\_heatmap\_testset\_posterior.pdf}: heatmaps of
...
The ASSIGN package allows a signature to be adapted to fit other biological
contexts, reducing the contribution of specific genes in the signature to better
match heterogeneity observed in the test dataset. Occasionally, adapting a
signature may reduce the importance of key signature genes. For example, if a
signature is created by overexpressing an oncogenic gene in a cell line, but
during the adaptation step, ASSIGN reduces the importance of that key gene, the
quality of the ASSIGN predictions may be reduced. Alternatively, if a gene in
the signature is associated with some other heterogeneity in the data, such as
smoking status, ASSIGN may adapt to differences in that gene, rather than the
actual desired signature activity predictions. To this end, we have added the
ability to provide a list of key genes to anchor in the signature, and genes to
exclude from the signature. ASSIGN accomplishes this by setting the probability
of inclusion into the signature to one for anchor genes, and zero for exclude
genes. The change in expression values can still adapt, increasing or reducing
the fold change associated with each gene in the signature, but the anchor genes
will always contribute to the final signature, and the exclude genes will not.

\subsection{Fraction of Upregulated Genes}

By default, ASSIGN bayesian gene selection chooses the signature genes with an
equal fraction of genes that increase with pathway activity and genes that
decrease with pathway activity. Use the pctUp parameter to modify this fraction.
Set pctUP to NULL to select the most significant genes, regardless of direction.
